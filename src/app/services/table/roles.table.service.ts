...
function sort(roles, column: SortColumn, direction: string) {
    if (direction === '' || column === '') {
        return roles;
    } else {
...
function matches(role, term: string, pipe: PipeTransform) {
    return role.name.toLowerCase().includes(term.toLowerCase())
        || role.createdAt.includes(term.toLowerCase());
}
...
export class RolesTableService {
    ...
    private _search(): Observable<SearchResult> {
        if (this.sharingData && this.sharingData.length) {
            ...
            // 1. sort
            let roles = sort(this.sharingData, sortColumn, sortDirection);
            // 2. filter
